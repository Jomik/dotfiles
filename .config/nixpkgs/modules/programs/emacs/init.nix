...
let
    ...
    config = mkIf config.enable {
      assembly =
        let
          ...
          mkCommands = vs: optional (vs != []) ":commands (${toString vs})";
        ...
        in
          concatStringsSep "\n  " (
            ...
            ++ optionals (cfg.general && config.general != "") [ ":general" config.general ]
  ...
  '' + optionalString cfg.diminish ''

    ;; For :diminish in (use-package).
    (require 'diminish)
  '' + optionalString cfg.bindKey ''

    ;; For :bind in (use-package).
    (require 'bind-key)
  '' + optionalString cfg.general ''
...
in {
  options.programs.emacs.init = {
    ...
    diminish = mkEnableOption ":diminish in use-package";
    bindKey = mkEnableOption ":bind in use-package";
    general = mkEnableOption ":general in use-package";
  ...
  config = mkIf (config.programs.emacs.enable && cfg.enable) {
    programs.emacs.extraPackages = epkgs:
      ...
      in [ epkgs.use-package ]
        ++ optional cfg.diminish epkgs.diminish
        ++ optional cfg.bindKey epkgs.bind-key
        ++ optional cfg.general epkgs.general
        ++ (
