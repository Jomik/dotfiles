...
6. **Draft the PR** — show the proposed title and body to the user for review. **Stop and wait for user approval before proceeding.**
7. **Push** — use the project's VCS to push the current change to the remote.
8. **Create PR** — pass the body inline using a single-quoted heredoc: `gh pr create --title "..." --body "$(cat <<'EOF' ... EOF)"`. The single-quoted `'EOF'` delimiter prevents shell interpretation of markdown `#` headings. Never run interactively, never write temp files. Always include `--assignee @me`.
