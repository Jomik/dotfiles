...
2. **Determine the repository** — infer from cwd (`gh repo view --json nameWithOwner -q .nameWithOwner`). Ask if targeting a different repo.
3. **Check for duplicates** — `gh issue list --state all -S "keyword"`. Use 2-3 distinctive terms from the title, not generic words — a vague search gives false confidence. If a likely duplicate exists, inform the user and ask whether to comment on it instead.
4. **Determine permission level** — `gh repo view --json viewerPermission -q .viewerPermission`. This controls which sections to include (see Template Rules below).
5. **Pick a template** — bug report or task, based on the user's description.
6. **Draft the issue** — fill in the template. Show the proposed title and body to the user for review. **Stop and wait for user approval before proceeding.**
7. **Create** — pass the body inline using a single-quoted heredoc: `gh issue create --title "..." --body "$(cat <<'EOF' ... EOF)"` (never run interactively, never write temp files). The single-quoted `'EOF'` delimiter prevents shell interpretation of markdown `#` headings. Include `--label` if the project uses them (`gh label list` to check) and `--assignee` if requested.
8. **Report back** — print the issue URL.
