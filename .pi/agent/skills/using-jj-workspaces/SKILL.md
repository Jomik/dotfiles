...
Use together with the `using-jj` skill. All normal jj rules still apply.

## Commands

```bash
# Create — always pass -r so the base is explicit.
# Prefer -m so the new working-copy commit is clearly labeled.
jj workspace add ../ws-name -r 'trunk()' -m "description of work"
...
```bash
jj workspace add ../ws-agent-task -r 'trunk()' -m "task: agent's work"
